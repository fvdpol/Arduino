...
TagExchangeJSONStream tagExchangeJSON(&Serial, &jsonStream);
...
void setup(void) 
{
  ...
  // test transmission of lifesign message
  ...
  tagExchangeJSON.registerFloatHandler(&JSON_TagUpdateFloatHandler);
  tagExchangeJSON.registerLongHandler(&JSON_TagUpdateLongHandler);
}  
...
{         
  ...
  BlinkRF12Led();
  tagExchangeJSON.publishFloat(tagid, value);
}
...
void RF12_TagUpdateLongHandler(int tagid, unsigned long timestamp, long value)
{
  ...
  BlinkRF12Led();
  tagExchangeJSON.publishLong(tagid, value);
}
...
// callback function: executed when a new tag update of type float is received
void JSON_TagUpdateFloatHandler(int tagid, unsigned long timestamp, float value)
{         
  if (debug_txt) {  
    Serial.print(F("JSON Float Update Tagid="));
    Serial.print(tagid);
...
// callback function: executed when a new tag update of type long is received
void JSON_TagUpdateLongHandler(int tagid, unsigned long timestamp, long value)
{
  if (debug_txt) {  
    Serial.print(F("JSON Long Update Tagid="));
    Serial.print(tagid);
...
void loop(void) {
  ...
  switch (scheduler.poll()) {
    ...
    case TASK_LIFESIGN:
      ...
      tagExchangeJSON.publishLong(100 + NODE_ID ,  millis());
